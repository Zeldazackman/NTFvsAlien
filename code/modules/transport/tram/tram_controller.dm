...
/datum/tram_mfg_info/New(specific_transport_id)
	...
	mfg_date = "[GAME_YEAR]-[time2text(world.timeofday, "MM-DD")]"
...
/datum/transport_controller/linear/tram/proc/crash_fx()
	playsound(source = nav_beacon, soundin = 'sound/vehicles/car_crash.ogg', vol = 100, vary = FALSE, falloff = DEFAULT_TRAM_LENGTH)
...
/datum/transport_controller/linear/tram/proc/announce_malf_event()
	priority_announce("Our automated control system has lost contact with the tram's onboard computer. Please stand by, engineering has been dispatched to the tram to perform a reset.", "Engineering Division")
 ...
 */
/datum/transport_controller/linear/tram/proc/rod_collision()
...
/obj/machinery/transport/tram_controller/attackby(obj/item/weapon, mob/living/user, params)
	if(user.a_intent == INTENT_HARM || cover_open)
		return ..()
...
/obj/machinery/transport/tram_controller/attack_hand_alternate(mob/living/user)
	. = ..()
	if(!has_cover)
		return TRUE
