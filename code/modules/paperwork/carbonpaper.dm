...
/obj/item/paper/carbon/verb/removecopy()
	...
	if (copied == 0)
		...
		copycontents = replacetext(copycontents, html_encode("<font face=\"[c.deffont]\" color="), html_encode("<font face=\"[c.deffont]\" nocolor="))	//state of the art techniques in action
		copycontents = replacetext(copycontents, html_encode("<font face=\"[c.crayonfont]\" color="), html_encode("<font face=\"[c.crayonfont]\" nocolor="))	//This basically just breaks the existing color tag, which we need to do because the innermost tag takes priority.
