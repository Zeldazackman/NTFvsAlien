...
/mob/living/post_crush_act(mob/living/carbon/xenomorph/charger, datum/action/ability/xeno_action/ready_charge/charge_datum)
	...
	var/charge_speed = CHARGE_SPEED(charge_datum)
	switch(charge_datum.charge_type)
		if(CHARGE_CRUSH)
			Paralyze(charge_speed * (2 SECONDS))
			Stagger((charge_speed + 1) * (2 SECONDS))
		if(CHARGE_BULL_HEADBUTT)
			Paralyze((charge_speed * (1.5 SECONDS)))
			Stun((charge_speed * (1.5 SECONDS) + (2 SECONDS)))
		if(CHARGE_BULL)
			Paralyze(0.2 SECONDS)
			Stun(2.2 SECONDS)
		if(CHARGE_BULL_GORE)
			adjust_stagger(charge_speed SECONDS)
			adjust_slowdown(charge_speed)
